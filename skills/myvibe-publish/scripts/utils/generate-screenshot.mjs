...
export async function generateScreenshot(options) {
  const { hub = VIBE_HUB_URL_DEFAULT, port = DEFAULT_PORT } = options
...
async function takeScreenshot(url, outputPath) {
  ...
  try {
    ...
    if (error.killed) {
      throw new Error(`Failed to open URL in agent-browser: timed out after 30s`, { cause: error })
    }
  ...
  try {
    ...
    throw new Error(`Failed to take screenshot: ${error.message}`, { cause: error })
  }
