...
| 特性 | 说明 |
|------|------|
| **Git 仓库发布** | 直接从 Git 仓库 URL 克隆并发布 |
| **智能检测** | 自动识别 Static、Vite、Next.js、Astro、Nuxt、Remix、SvelteKit、Angular、Solid.js、Gatsby、Hugo、Jekyll、MkDocs、Docusaurus、Monorepo |
...
```
/myvibe-publish --repo https://github.com/user/project 从 Git 仓库发布
```
...
| `--repo <url>` | `-r` | Git 仓库 URL，克隆后发布 |
| `--branch <ref>` | `-b` | 分支、标签或 commit hash |
| `--path <subdir>` | `-p` | 仓库内子目录（适用于 monorepo）|
| `--git-token <token>` | | 私有仓库 HTTPS 克隆的访问令牌 |
